...
class ManageCCViewController: UIViewController {
    ...
    @IBAction func displayCategories(_ sender: UIButton) {
        ...
        // Retrieve the list of categories from the data model
        let categories = dataModel.categories
    ...
    @IBAction func selectCategoryView(_ sender: UIButton) {
        ...
        if let selectedCategory = dataModel.categories.first(where: { $0.category_Id == categoryId }) {
            ...
            // Pass the selected Category object to the next view controller
